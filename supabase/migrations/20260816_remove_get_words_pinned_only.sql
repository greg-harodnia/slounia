-- Remove the pinned_only filter and the pinned_at sort branch from get_words:
-- pinned words are now derived client-side from the full list, so the
-- homepage no longer needs a dedicated "pinned section" query, and nothing
-- else ever passed pinned_only / sorted by pinned_at.
