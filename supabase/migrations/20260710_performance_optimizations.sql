-- Add pinned_only param to get_words (DROP + CREATE to avoid overload issue)

-- Drop the current 8-param overload
DROP FUNCTION IF EXISTS get_words(TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, TEXT[], BOOLEAN);

-- Recreate with pinned_only parameter
...
-- Lightweight single-word fetch (new function, no conflict)
...
-- Batch reorder translations (new function, no conflict)
