<script lang="ts">
	...
	// The PWA web manifest is generated by vite-plugin-pwa; link it from the
	// head (pwaInfo is a virtual module, see vite-plugin-pwa docs for SvelteKit).
	...
	onMount(() => {
		// SvelteKit has no static HTML entry, so vite-plugin-pwa can't inject
		// the registerSW script automatically; register the service worker
		// explicitly here (dynamic import keeps it client-only).
		if (pwaInfo) {
